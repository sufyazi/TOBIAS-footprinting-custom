...
for dataset in "${datasets[@]}"; do
	...
	for sample_dir in "${dataset_path}"/*; do
		...
		if [ ! -d "${sample_dir}" ]; then
			...
			if rsync -avPhz "${bam_files[@]}" "/home/msazizan/cargospace/tobias-raw-input-files/${dataset}/${substring}/"; then
				cd /home/msazizan/cargospace/tobias-raw-input-files || exit 1
				# rsync the folder to Aspire
				rsync -havPz --relative "${dataset}/${substring}"/ suffiazi@aspire2antu.nscc.sg:/home/users/ntu/suffiazi/scratch/inputs/atac-croo-out/
			fi
		fi
	done
